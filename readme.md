# Grafana reporter <img style="float: right;" src="https://travis-ci.org/ripoUGC/reporter.svg?branch=master">
...
Get the source files and dependencies:
    ...
    go get github.com/ripoUGC/reporter/...

Build and install `grafana-reporter` binary to `$GOPATH/bin`:

    go install -v github.com/ripoUGC/reporter/cmd/grafana-reporter
...
Also see [this issue](https://github.com/ripoUGC/reporter/issues/50) for an example. 
...
A Docker image [is available](https://hub.docker.com/r/ripoUGC/grafana-reporter/). To see available flags:

    docker run ripoUGC/grafana-reporter --help

To run with default flags, use `--net` to enable Docker to connect to Grafana at `localhost:3000`:

    docker run -p 8686:8686 --net="host" ripoUGC/grafana-reporter
...
If you also have `Make` and `Docker-compose` installed, you can run a simple local orchestration of Grafana and Grafana-reporter:
     ...
     go get github.com/ripoUGC/reporter/ ...
     cd $GOPATH/src/github.com/ripoUGC/reporter
     make compose-up
...
The unit tests can be run using the go tool:
    ...
    go test -v github.com/ripoUGC/reporter/...

or, the [GoConvey](http://goconvey.co/) webGUI:

    ./bin/goconvey -workDir `pwd`/src/github.com/ripoUGC -excludedDirs `pwd`/src/github.com/ripoUGC/reporter/tmp/
